...
const ResponsiveGridLayout = () => {
    ...
    return (
        ...
        >
            {layout.map((item) => (
                <div key={item.i} className="bg-white grid place-content-center hover:bg-cyan-500 hover:dark:bg-cyan-500 cursor-move dark:bg-slate-800 border border-cyan-500/50 rounded-md">
                    {item.i}
                </div>
            ))}
        </GridLayout>
