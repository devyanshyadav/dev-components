...
type Tag = { key: number; name: string };
...
const DevChipInput = ({ defaultValues, onChange, trigger = "Enter", tagLength = 14, textLength = 20 }: TagsInputProps) => {
  const [tags, setTags] = useState<Tag[]>(defaultValues);
  ...
  const handleEnter = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (tags.some(tag => tag.name === inputValue)) return alert("Tag already exists");
    if (inputValue.length > textLength) return alert(`Tag must be less than ${textLength} characters`);

    const newTag: Tag = { key:Date.now(), name: inputValue };
