  ...
  return (
    <div className='w-full md:max-w-sm bg-rtlLight p-3 dark:bg-rtlDark border border-accentNeon/50 rounded-xl'>
      <span className='flex items-center gap-1 p-1 rounded-full border border-accentNeon/50'>
        ...
        />
        <DevClipboardV1 setCopied={setCopy} className='bg-accentNeon text-white rounded-full p-1 px-3 active:opacity-80' textClip={shareUrl}>
      ...
      <div role="icon-group" className='flex flex-wrap gap-2 mt-2'>
        {shareConfig && shareConfig.map((share, index) => (
          <share.shareWrapper url={shareUrl} key={index} >
            <share.shareIcon className="border border-accentNeon/50 hover:border-accentNeon rounded-full p-[2px] hover:scale-105 transition-all" size={size} round={isRounded} />
          </share.shareWrapper>
        ))}
      </div>
    </div>
