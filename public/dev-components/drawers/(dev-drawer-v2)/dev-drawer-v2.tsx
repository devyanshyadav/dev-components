...
export default function DevDrawerV2({ children = null, position = 'right', title, openBtn = null }: DevDrawerProps) {
    ...
    return (
        <Drawer.Root direction={position}>
            ...
            <Drawer.Portal>
                ...
                <Drawer.Content className={`z-50 bg-rtlLight shadow-[3px_200px_0px_1px_rgba(0,0,0,0)] border-accentNeon/50 dark:bg-rtlDark fixed flex flex-col  ${getPositionClasses[position]}`}>
                    {
                        title && <h2 className="border-b p-3 border-accentNeon/50 text-accentNeon font-semibold flex-shrink-0">
                            {title}
                        </h2>
                    }
