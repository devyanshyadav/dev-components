    ...
    <div className='w-64 bg-LIGHT dark:bg-DARK rounded-lg border border-ACCENT/40 p-3 flex flex-col gap-3 max-h-[500px] overflow-hidden'>
      ...
      <div
        className={`
          ...
          ${fileLimitExceeded ? 'border-red-500 bg-red-500/30' : 'border-ACCENT/50'}
        `}
      ...
      >
        ...
        <div className='flex items-center flex-col justify-center gap-2'>
          ...
          <button className='bg-ACCENT p-1.5 px-4 rounded-lg text-white' disabled={disabled}>
            Browse
          </button>
        </div>
      </div>
      ...
      {uploadedFiles.length > 0 && (
        <div className="space-y-3 flex-grow overflow-auto overflow-x-hidden">
          {uploadedFiles.map((fileObj, index) => (
            <div key={index} className="border-2 border-ACCENT/20 relative p-3 rounded-lg overflow-hidden flex flex-col">
              <div className="flex items-center justify-between mb-2 ">
                <div className="flex items-center space-x-2">
                  <span className='flex-shrink-0 text-lg'>{handleFileIcon(fileObj.file.type)}</span>
                  <span className="text-sm text-ellipsis overflow-hidden flex-grow">{fileObj.file.name}</span>
                </div>
                <button className='absolute top-1 hover:text-red-500 right-1' onClick={() => removeFile(fileObj.file)}>
                  <IoCloseSharp />
                </button>
              </div>
      ...
      {uploadedFiles.length > 0 && (
        <div className="mt-4 flex justify-between">
          <button className='p-1.5 px-4 rounded-lg text-white bg-ACCENT' onClick={onSubmit}>Submit</button>
          <button className='text-red-500 bg-red-500/30 p-1.5 px-4 rounded-lg' onClick={clearAllFiles}>Clear All</button>
        </div>
      )}
    </div>
  );
