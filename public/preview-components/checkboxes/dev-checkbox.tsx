...
type devCheckboxProps = {
  checked: boolean;
  isChecked: (checked: boolean) => void;
};

const DevCheckbox = ({ checked, isChecked }: devCheckboxProps) => {
  return (
    <div className="flex gap-2  items-center justify-start cursor-pointer w-40 border border-cyan-500/30 p-3 rounded-md">
      <span
        id="dev-checkbox"
        className={`border-2 border-cyan-500 text-white w-5  aspect-square grid place-content-center rounded-md cursor-pointer ${
          checked ? "bg-cyan-500" : "bg-cyan-500/20"
        }`}
      >
        {checked && (
          <svg
